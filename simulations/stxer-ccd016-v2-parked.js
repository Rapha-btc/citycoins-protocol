...
async function main() {
  ...
  tx("P4 stranger readmit-token-x(vault) on a full side -> u1010", STRANGER, MKT_ID, "readmit-token-x", [contractPrincipalCV(DEPLOYER, VAULT), UPD], "(err u1010)");
  ...
  tx("P5 jing-reclaim again -> the market has nothing (err)", STRANGER, VAULT_ID, "jing-reclaim", [], (v) => String(v).startsWith("(err"));
